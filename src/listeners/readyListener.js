...
export async function onReady(client) {
  ...
  if (ticketConfig.TARGET_MESSAGE_ID) {
    ...
    console.warn(
      '⚠️ `TARGET_MESSAGE_ID` nicht gefunden! Warte 5 Sekunden, dann erneut versuchen...'
    );
    setTimeout(async () => {
      if (ticketConfig.TARGET_MESSAGE_ID) {
        ...
        console.error(
          '❌ `TARGET_MESSAGE_ID` konnte nicht automatisch gesetzt werden.'
        );
      }
    }, 5000);
  }
  ...
  if (botConfig.status && botConfig.statusType) {
    ...
    console.log(
      `🎮 Status gesetzt auf: ${botConfig.statusType} ${botConfig.status}`
    );
  }
}
