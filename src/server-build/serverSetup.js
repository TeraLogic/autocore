...
export async function createOrFetchTicketMessage(guild) {
    ...
    if (ticketConfig.TARGET_MESSAGE_ID) {
        try {
            const existingMessage = await channel.messages.fetch(ticketConfig.TARGET_MESSAGE_ID);
            
            if (existingMessage) {
                if (existingMessage.content !== ticketText) {
                    console.log(`📝 Ticket-Nachricht aktualisieren, da sich der Text geändert hat.`);
                    await existingMessage.edit(ticketText);
                }
                return;
            }
        } catch (error) {
            console.warn(`⚠️ Ticket-Nachricht mit ID ${ticketConfig.TARGET_MESSAGE_ID} existiert nicht mehr. Erstelle eine neue.`);
    ...
    if (needsNewMessage) {
        ...
        saveConfig();
        console.log(`✅ Neue Ticket-Nachricht erstellt & gespeichert: ${newMessage.id}`);
    }
}
...
export async function setupServer(guild) {
    ...
    const ticketChannel = await getOrCreateChannel(guild, 'ticket', ChannelType.GuildText, ticketCategory);
